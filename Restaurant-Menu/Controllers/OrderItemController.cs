...
public class OrderItemController : ControllerBase
{
    ...
    {
        _logger.LogInformation($"Fetching order items for OrderID={orderId}");

        var orderItems = await _orderItemRepository.GetOrderItemsByOrderIdAsync(orderId);

        if (orderItems == null || !orderItems.Any())
        {
            _logger.LogWarning($"No order items found for OrderID={orderId}");
    ...
    public async Task<ActionResult> AddOrderItemsToOrder(int orderId, [FromBody] List<OrderItem> orderItems)
    {
        if (orderItems == null || !orderItems.Any())
        {
        ...
        {
            item.OrderID = orderId; // Ensure correct OrderID assignment
    ...
    public async Task<IActionResult> UpdateOrderItem(int orderId, int id, [FromBody] OrderItem orderItem)
    {
        ...
        {
            return NotFound($"OrderItem ID={id} not found in OrderID={orderId}.");
        }

        try
        {
            existingItem.UpdatedBy = "DefaultUser"; // Modify this to get the actual logged-in user
    ...
    {
        _logger.LogInformation($"Attempting to delete OrderItem ID={id} from OrderID={orderId}");

        var orderItem = await _orderItemRepository.GetOrderItemByIdAsync(id);
        if (orderItem == null || orderItem.OrderID != orderId)
        {
            _logger.LogWarning($"OrderItem ID={id} not found in OrderID={orderId}.");
            return NotFound($"OrderItem ID={id} not found in OrderID={orderId}.");
        }
        ...
        try
        {
            ...
            _logger.LogInformation($"Successfully deleted OrderItem ID={id} from OrderID={orderId}");
            return NoContent();
        }
        catch (Exception ex)
        {
